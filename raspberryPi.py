...
# 라인트레이싱을 진행하고 있는 구간 -> 1번부터 5번까지 존재
global Line_tracing_number
...
#color
yellow1 = np.array([11, 123,68])    #min of yellow
yellow2 = np.array([57,255,255])#max of yellow
yellow_chance = 2 #sensitive (default = 2)

bell_yellow1 =  np.array([0, 108,75]) 
bell_yellow2 = np.array([44,289,158])
bell_yellow_chance = 2 #sensitive (default = 2)

blue1 = np.array([100, 140,53])    #min of blue (default : [120-10, 30,30])
blue2 = np.array([124, 203,188])   #max of blue (default : [120+10, 255,255])
blue_chance = 2 #sensitive (default = 2)

green1= np.array([49, 26,66]) 
green2 = np.array([84, 173,188]) 
green_chance= 2 #sensitive (default = 2)

black1 = np.array([0, 103,0])    #min of blue (default : [120-10, 30,30])
black2 = np.array([40, 125,44])   #max of blue (default : [120+10, 255,255])
black_chance = 2 #sensitive (default = 2)

grap_blue_color1 = np.array([70, 112,60])    #min of blue (default : [120-10, 30,30])
grap_blue_color2 = np.array([122, 203,120])   #max of blue (default : [120+10, 255,255])
...
ah_eh_rev_sensitive = 25 #ㅓ,ㅏ판단할 때 둔감도(default 20)


angle_yellow1 = np.array([16, 80,140])
angle_yellow2= np.array([90, 255,255])
right_angle = 1.3 # (정상 최대 오른각도   default = 1)
left_angle = -1.3  #right_angle * (-1) 권장



T_yellow1 = np.array([16, 80,140])
T_yellow2= np.array([90, 255,255])
...
T_yellow1, T_yellow2= yellow1, yellow2
angle_yellow , angle_yellow2= yellow1, yellow2
mid_yellow1 , mid_yellow2 = yellow1, yellow2
...
################nwes_detector################
nwes_sensitive=2
def nwesdetect(cap,letter_sensitive):
    global Letter_cnt
    global Line_tracing_number 
    charlist=[]
    ################[Can't,  N,  W,  E, S]#########################
    lst_sensitive = [0, 0,      0,     0,       0]
    past = 'k'
    sensitive = letter_sensitive
        ...
        while (sensitive not in lst_sensitive):
            ...
            if imgchar=='': # 못 찾으면
                ...
                if imgchar.find('N')!=-1: #N이면
                    ...
                    print(f"Find {imgchar} is Not NWES")
                    if past=='': # 연속으로 못찾음
                        lst_sensitive[0]+=1
                    else: #연속아님
                        lst_sensitive=[0,0,0,0,0]
...
################abcd detector################        
abcd_sensitive=3
def abcddetect(cap,abcd_sensitive):
    global Letter_cnt
    global Line_tracing_number
    charlist=[]
    ################[Can't,  A,  B,  C, D]#########################
        ...
        while (sensitive not in lst_sensitive):
            ...
            if imgchar=='': # 못 찾으면
                ...
                if imgchar.find('A')!=-1: #A이면
                    ...
                    print(f"Find {imgchar} is Not ABCD")
                    if past=='': # 연속으로 못찾음
                        lst_sensitive[0]+=1
                    else: #연속아님
                        lst_sensitive=[0,0,0,0,0]
...
def detect_angle(cap, yellow1, yellow2):    
    while (True):
    ...
    ##    print(ys,xs)
        ...
        dst = cv2.Canny(imgray, 75, 150, None, 3) #canny처리하기
    ...
    ###################################################################
        ...
        linesP = cv.HoughLinesP(dst, 1, np.pi/180, 30, None, 50, 150) #확률적허프변환
...
def where_is_blue(cap,where_blue1,where_blue2,where_blue_center_sensitive,) -> 9:
    ...
    while(True):
        ...
        cam = cv2.GaussianBlur(imgray, (3, 3), 0)  # 가우시안 블러
        ret, cam_binary = cv2.threshold(cam, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)  # 이진화
        contours, hierarchy = cv2.findContours(cam_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 컨투어
        ...
        while ser.inWaiting() > 0:
            ...
            if RX == 16:
                ...
                if RX == Arrow_detect:
                    ...
                    flag = arrow(cap,arrow_sensitive)
                ...
                # ----- 글자 인식 신호 ------
                    ...
                    if Letter_cnt == 3:
                        print("---------------- 글자 인식::방위 ------------------")
                        flag=nwesdetect(cap,nwes_sensitive)
                    
                    elif Letter_cnt == 2:
                        print("---------------- 글자 인식::1번째 방 ------------------")
                        flag=abcddetect(cap,abcd_sensitive)
                ...
                # : 0개일 때 -> 들려야하는 미션방이 0개 남은 상태
                    ...
                    if Line_tracing_number == 1:
                        print("---------------- 1번째 지점 라인 트레이싱 ------------------")
                    elif Line_tracing_number == 2:
                        print("---------------- 2번째 지점 라인 트레이싱 ------------------")
                    elif Line_tracing_number == 3:
                        print("---------------- 3번째 지점 라인 트레이싱 ------------------")
                    elif Line_tracing_number == 4:
                        print("---------------- 4번째 지점 라인 트레이싱 ------------------")
                    elif Line_tracing_number == 5:
                        print("---------------- 5번째 지점 라인 트레이싱 ------------------")
                    ...
                    TX_data(ser, flag) # 중앙정렬 결과 값 전송
                    print(f"TX={flag} ")
                    
                    print("-----angle------")

                    flag = detect_angle(cap, yellow1, yellow2)
                ...
                # 'T'인식 코드
                    ...
                    flag = detect_T(cap, T_yellow1,T_yellow2,center_sensitive,T_sensitive,T_order,length_T)
                ...
                # # ----- 초록색 인식 신호 ------
                    ...
                    flag=Green_color_detect(cap, green1, green2, green_chance)
